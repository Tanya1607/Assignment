...
async def initialize_documents(force_reinit: bool = False):
    try:
        ...
        print(f"Looking for documents in: {root_dir}")
        print(f"Force reinitialize: {force_reinit}")

        processed_files = []
        errors = []

        nec_file = os.path.join(root_dir, "2017-NEC-Code-2 (1).pdf")
        ...
        wattmonk_file = os.path.join(root_dir, "Wattmonk Information.docx")
